export const receiptParserXY = function (gcp){
    ...
    const unsortedReceiptArray = gcp[0]['textAnnotations'].splice(1) 
    // console.log('PARSER XY GCP OBJ', gcp.length)
    // console.log('UNSORTED RECEIPT ARR',typeof unsortedReceiptArray, unsortedReceiptArray)
    ...
    for (let i=0; i< receiptArray.length; i++){
        ...
        if (receipt.items.length){
            if (!receipt.tax){
                let receiptWordDescription = receiptArray[i-1].description.toLowerCase()
              //  console.log('PREV WORD: ',receiptWordDescription,'CURRENT OBJ:', textObj.description)
                if (receiptArray[i-1].description.toLowerCase() === 'tax' && validatePrice(textObj.description.trim())){
                        receipt.tax.push(textObj)
                }
            } 
            if (receipt.total.length === 0){
                if (receiptArray[i-1].description.toLowerCase() === 'total' && validatePrice(textObj.description.trim())){ 
                        receipt.total.push(textObj) 
                }
            }
        }
    }
